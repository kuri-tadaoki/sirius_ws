...
public:
  ...
  explicit Nav2Client(): Node("nav2_send_goal"), count_(0),renew_(0)
  {
    //アクション Client の作成

    this->client_ptr_  = rclcpp_action::create_client<NavigateToPose>(this, "navigate_to_pose");
    file_path_ = "/home/sirius/sirius_ws/src/sirius_navigation/config/15hall.yaml";
  ...
  void sendGoal(void) {
    ...
    //count_ ++;
	    ...
	    RCLCPP_INFO(get_logger(), "reach the end of goal!!!!");
    }
  ...
  void feedbackCallback(GoalHandleNavigateToPose::SharedPtr,const std::shared_ptr<const NavigateToPose::Feedback> feedback)
  {
    RCLCPP_INFO(get_logger(), "Distance remaininf = %f", feedback->distance_remaining);
        ...
        if (feedback->distance_remaining < 0.35&& renew_==1){
	    count_++;
	    renew_=0;
