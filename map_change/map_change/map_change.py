...
class MapChange(Node):
    ...
    def odom_callback(self, odom_msg):
        if self.wp_x is not None and self.wp_y is not None:
            ...
            self.get_logger().info('Robot distance: %f' % distance)

            if distance <= 1.5:
                subprocess.run(["bash", "robo_node_kill.bash"])
                subprocess.run(["bash", "nav2_nodekill.bash"])
                subprocess.run(["bash", "robo_bringup.bash"])
...
def main(args=None):
    ...
    rclpy.spin(map_change)

    map_change.destroy_node()
    rclpy.shutdown()
