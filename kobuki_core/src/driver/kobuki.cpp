...
ecl::Angle<double> Kobuki::getHeading0() const
{
...
// ecl::Angle<double> Kobuki::getHeading2() const
// {
//   ecl::Angle<double> heading;
//   // raw data angles are in hundredths of a degree, convert to radians.
//   heading = diff_drive.heading();
//   return ecl::wrap_angle(heading);
// }
...
void Kobuki::resetOdometry()
{
  ...
  heading_offset = (static_cast<double>(inertia.data.angle) / 100.0) * ecl::pi / 180.0;
  heading_offset = 0.0;// inoue kaeta oryaaa--
}
 ...
 */
void Kobuki::updateOdometry0(
    ecl::linear_algebra::Vector3d &pose_update,
    ecl::linear_algebra::Vector3d &pose_update_rates)
{
  sig_debug.emit("updateOdometry0");
  // if (start_odom_flg)
  // {
  //   if (core_sensors.data.left_encoder == (uint16_t)65536 && core_sensors.data.right_encoder == (uint16_t)65536)
  //   {
  //     core_sensors.data.left_encoder = (uint16_t)0;
  //     core_sensors.data.right_encoder = (uint16_t)0;
  //   }
  //   start_odom_flg = false;
  // }
  ...
  std::cout << "left_encoder right_encoder : " << core_sensors.data.left_encoder <<  " " << core_sensors.data.right_encoder << std::endl;
  std::cout << "right_encoder : " << core_sensors.data.right_encoder << std::endl;
}
